<template>
    <teleport to="body">
        <div class="modal backdrop-brightness-50 fixed w-screen h-screen top-0 md:left-[8.33334%] flex justify-center items-center z-50"
            v-if="isOpen">
            <EditNoteComponent :mapNote="selectedMapNote" @close="isOpen = false"></EditNoteComponent>
        </div>
    </teleport>
    <div class="h-2/3 w-full relative overflow-x-hidden" @click="clicked"
        :style="{ background: `url(${selectedImage.url}) no-repeat`, backgroundPosition: 'center center', backgroundSize: 'cover' }"
        v-if="selectedImage">
        <div v-for="pin in mapPins" :key="pin.id"
            ...
            @click="editNotes($event, pin); isOpen = true">


        </div>
    </div>
    <div class="w-full h-3/5 flex flex-col justify-center items-center text-text-color text-2xl" v-else> Select
        a
        map.
        <div class="p-10">
            <div class="dropdown inline-block relative group hover:block">
                <button class=" bg-accent text-text-color font-semibold py-2 px-4 rounded inline-flex items-center">
                ...
                <ul class="dropdown-menu absolute hidden group-hover:block  text-primary pt-1">
                    ...
                    <li v-for="image in mapImages" :key="image.id"><a
                            class="rounded-t bg-text-color hover:bg-gray-400 py-3 px-4 block"
                            @click="selectImage(image)">Map Number</a>
                    </li>
                </ul>
            </div>
...
const selectedMapNote = ref(null);
